...
You can install ASAutotest by simply symlinking the executable into
your PATH:

  $ ln -s ~/asautotest/bin/asautotest.rb ~/bin/asautotest
...
If you have additional source directores, use the <tt>--source</tt>
option (or <tt>-I</tt> for short) to add them:
...
be able to connect to ASAutotest:
  ...
  $ sudo ~/asautotest/bin/policy-server.rb

This will just keep running, so you may want to start it in a screen:

  $ screen sudo ~/asautotest/bin/policy-server.rb
